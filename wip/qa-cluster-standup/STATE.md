...
**Track 1 — Storage / Tier-3 — ✅ DONE 2026-07-14.** Wired via GitOps (2 commits: flux-platform op-qa `50cee8f` adds `infrastructure/local-path-storage/`; cluster-repo master `1287c9b` adds local-path-storage→rook-ceph-operator→rook-ceph-cluster Kustomizations to `clusters/op-usxpress-qa/flux-system/infra.yaml`). Result: all 4 Kustomizations Ready; local-path-provisioner Running; **Ceph HEALTH_OK** (3 mons quorum a/b/c, mgr active, **5 OSDs** up/in on application `/dev/sdb`, spread across zones a/b/c, 2.4 TiB, `replicapool` Ready, 41 pgs active+clean); SCs `local-path`+`ceph-block`+`ceph-fs`+`ceph-bucket` present. **grafana + prometheus PVCs Bound on ceph-block**; prometheus 2/2 Running. The codified local-path (Talos `/var` path + privileged ns label) worked first try — mon PVCs bound, no helper-pod denial. **grafana `CreateContainerConfigError` — FIXED 2026-07-14.** Cause: op-qa grafana ExternalSecrets (`externalsecret.yaml`, `externalsecret-azure-ad.yaml`) hardcoded **Dev** SM paths `op-usxpress-dev/platform/grafana[/azure-ad]` → QA account SM (527101283767, us-east-2, store `default` via JWT/IRSA — proven working by etcd-backup talosconfig ES) returned "Secret does not exist". Fix (2 parts): (1) seeded QA SM `op-usxpress-qa/platform/grafana` (admin user + generated pw) and `.../grafana/azure-ad` (**PLACEHOLDER** client_id/secret — real Entra QA app-reg is a follow-up; no Azure access, Dev creds won't work for QA redirect URI); (2) repointed op-qa manifests dev→qa (flux-platform op-qa `11ede5c`). Result: both ExternalSecrets synced, **grafana 3/3 Running**. **QA STORAGE + PLATFORM STACK NOW GREEN.**

### Other stale `op-usxpress-dev` refs on op-qa (from git grep, triaged 2026-07-14) → INFRA-1589 sweep
- **`external-dns` `txtOwnerId: iaac-talos/us-east-2/op-usxpress-dev`** — LATENT BUG: QA external-dns claims DNS TXT ownership as *dev* → could conflict with Dev over shared records. Fix (change to op-usxpress-qa) is a real follow-up.
