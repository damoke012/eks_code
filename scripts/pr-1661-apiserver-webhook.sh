...
# This script cd's into the iaac-talos work repo, so resolve our own repo first.
...
if [ "$PUSH" = "yes" ]; then
  # The PR body is a FILE, not a shell string. On 2026-08-24 it was a "..."
  # argument; the disclosure table used bare backticks, so the shell ran every
  # cell as a command -- it executed octopus/bento-import.py and
  # octopus/apply-bootstrap-perms.sh, which calls `aws iam put-role-policy`.
  # It reached `aws sts get-caller-identity` before dying on absent
  # credentials. A PR-authoring script must not be able to run anything.
  BODYFILE="$REPO_ROOT/scripts/pr-1661-body.md"
  [ -f "$BODYFILE" ] || { echo "   !! missing $BODYFILE" >&2; exit 1; }

  # Fail closed if any expansion happened anyway: these must survive verbatim.
  for lit in 'role/*-${CLUSTER_NAME}-*' '`deploy.ps1`' '`BENTO_PASSWORD`'; do
    grep -qF -- "$lit" "$BODYFILE" || {
      echo "   !! PR body lost '$lit' -- something expanded it. Not creating the PR." >&2
      exit 1
    }
  done
  echo "   PR body: $(wc -l < "$BODYFILE") lines, literals intact"
